...
def visualize(root_dir, mesh_names, simp_map, mesh_folder=None):
    ...
    state = {
        "mesh_idx":       [0],
        "simp_idx":       [0],
        # Two-stage reload to avoid triggering heavy work mid-frame:
        #   needs_reload  → set True when a dropdown changes
        #   reload_frame  → counts down 1 frame so polyscope finishes drawing
        #                   before we clear/register structures
        "needs_reload":   [True],
        "reload_frame":   [0],        # frames remaining before actual reload
        "data":           [{}],
        "sel_idx":        [-1],
        "sel_changed":    [False],
        "bnd_verts":      [set()],
    ...
    def current_mesh():
        return mesh_names[state["mesh_idx"][0]]

    def current_simps():
        return simp_map[current_mesh()]
    ...
    def reload():
        ...
        if 'sample_points' in d:
            pc = ps.register_point_cloud("Sample Points", d['sample_points'])
        ...
        if 'simp_spheres' in d:
            ...
            if 'simp_faces' in d and len(d['simp_faces']) > 0:
                ...
                m2.set_color(C_SIMP_MESH);  m2.set_transparency(0.30)
    ...
    def callback():
        ...
        # ── dropdowns ─────────────────────────────────────────────────────────
        psim.TextUnformatted("── Select ──────────────────────")
        psim.Separator()

        changed_m, new_m_idx = psim.Combo("Mesh", state["mesh_idx"][0], mesh_names)
        if changed_m and new_m_idx != state["mesh_idx"][0]:
            state["mesh_idx"][0]     = new_m_idx
            state["simp_idx"][0]     = 0
            state["needs_reload"][0] = True
            state["reload_frame"][0] = 2    # wait 2 frames before reloading
